...
class GraphFeatureTokenizer(torch.nn.Module):
    ...
    def __init__(
        ...
        # Linear projection to match input_feat_dim
        self.edge_proj = nn.Linear(3, input_feat_dim)
    ...
    def forward(
        ...
        data.num_graphs = batch.max().item() + 1
    ...
    def generate_node_identifiers(self, data: Batch) -> torch.Tensor:
        ...
        # dummpy_P = torch.zeros((x.size(0), self.d_p), device=device)
        # return dummpy_P
        ...
        for i in range(num_graphs):
            ...
            if self.method == "orf":
                ...
                G = torch.empty((num_nodes, num_nodes), device=device).normal_(
                    mean=0, std=1
                )
                # Orthonormalize
                ...
                raise ValueError(
                    "Invalid method for node identifiers. \
    ...
    def prepare_tokens(
        self,
        data: Batch,
        X_v_proj: torch.Tensor,
        X_e_proj: torch.Tensor,
        batch: torch.Tensor,
        ...
        attention_masks = torch.zeros(
            (num_graphs, seq_len), dtype=torch.bool, device=device
        )
...
class TokenGT(nn.Module):
    ...
    def __init__(
        ...
        self.tokenizer = GraphFeatureTokenizer(
            input_feat_dim, hidden_dim, method, d_p, d_e, use_graph_token
        )
    ...
    def forward(self, x, edge_index, edge_attr, batch) -> torch.Tensor:
        ...
        tokens, attention_masks = self.tokenizer(x, edge_index, edge_attr, batch)
        output = self.encoder(tokens, src_key_padding_mask=~attention_masks)

        return output, attention_masks


class BERTTokenGT(nn.Module):
    r"""Tokenized Graph Transformer (TokenGT) model from the `Pure Transformers
    are Powerful Graph Learners <https://arxiv.org/abs/2207.02505>`_ paper.
    Args:
        node_feat_dim (int): Dimension of node features.
    ...
    def __init__(
        ...
        config = BertConfig(
            ...
            is_decoder=False,
            use_flash_attn=True,
            add_pooling_layer=False,  # We just want the encoder
        )
    ...
    def forward(self, x, edge_index, edge_attr, batch) -> torch.Tensor:
        ...
        # Convert graphs into token sequences and attention masks
        tokens, attention_masks = self.tokenizer(x, edge_index, edge_attr, batch)
        output = self.encoder(hidden_states=tokens, key_padding_mask=attention_masks)
...
def create_batch(num_nodes, num_edges, input_feat_dim=16):
    ...
    for n_nodes, n_edges in zip(num_nodes, num_edges):
        ...
        edge_index = torch.tensor(np.random.choice(range(n_nodes), size=(2, n_edges)))
...
if __name__ == "__main__":
    ...
    print("TokenGT model ran successfully with Laplacian method.")
    print("Logits:", logits)
